...
function saveEmailToStore (id, isRead = false, envelope, parsedEmail) {
  ...
  const serialized = utils.clone(parsedEmail)
  serialized.id = id
...
function saveAttachment (id, attachment) {
  ...
  const contentId = attachment.contentId ??
    crypto.createHash('md5')
      .update(Buffer.from(attachment.filename, 'utf-8'))
...
function handleDataStream (stream, session, callback) {
  ...
  mailParser(stream, (err, mail) => {
    ...
    saveEmailToStore(id, false, envelope, mail)
  })
      ...
      files.forEach(function (file) {
        ...
        if (path.parse(file).ext === '.eml') {
          fs.readFile(filePath, 'utf8', async function (err, data) {
            ...
            if (err) {
              logger.error('Error when reading the file %s (%s)', filePath, err)
            } else {
              mailParser(data, (err, mail) => {
                if (err) {
                  logger.error('Error when parsing the file %s (%s)', filePath, err)
                }
                const envelope = {
                  from: mail.headers.from.length > 0 ? mail.headers.from[0].address: undefined,
                  to: mail.headers.to.map((t)=>t.address),
                  host: 'undefined',
                  remoteAddress: 'undefined'
                }

                saveEmailToStore(idMail, false, envelope, mail)
              })
            }
          })
        }
