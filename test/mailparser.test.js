...
describe('Mailparser General tests', () => {
  ...
  it('Reply To address', (done) => {
    const encodedText = 'Reply-TO: andris <andris@disposebox.com>\n' +
            'Subject: ÕÄÖÜ\n' +
            '\r' +
            '1234'
...
describe('Mailparser attachment encodings', () => {
  it('Quoted-Printable', (done) => {
    ...
    mailParser(encodedText, (_, mail) => {
      assert.strictEqual(Array.prototype.slice.apply(mail?.attachments[0]?.content || []).join(','), '0,1,2,3,253,254,255')
      done()
    })
  })
  ...
  it('8bit', (done) => {
    ...
    mailParser(encodedText, (_, mail) => {
      assert.strictEqual(Array.prototype.slice.apply(mail?.attachments[0]?.content || []).join(','), '195,149,195,132,195,150,195,156')
      done()
    })
  })
...
describe('Mailparser Attachment Content-Id', () => {
  it('Default', (done) => {
    ...
    mailParser(encodedText, (_, mail) => {
      assert.strictEqual(mail?.attachments[0]?.contentId, '7c7cf35ce5becf62faea56ed8d0ad6e4@mailparser')
      done()
    })
  })
...
describe('Mailparser Transfer encoding', () => {
  ...
  it('Missing Date header', (done) => {
    ...
    mailParser(encodedText, (_, mail) => {
      assert.ok(mail.date)
      assert.strictEqual(mail.headerLines.find((h) => h.key === 'date'), undefined)
      done()
    })
  })

  it('Received Headers', (done) => {
    const encodedText = 'Received: by 10.25.25.72 with SMTP id 69csp2404548lfz;\r\n' +
  ...
  it('X-Received Header', (done) => {
    const encodedText = 'X-Received: by 10.194.200.68 with SMTP id jq4mr7518476wjc.128.1423264531879;\r\n' +
            '        Fri, 06 Feb 2015 15:15:31 -0800 (PST)\r\n' +
            'Date: Fri, 6 Feb 2015 16:13:51 -0700 (MST)\r\n' +
            '\r\n' +
            '1cTW3A=='
...
describe('Mailparser charset handling', () => {
  const cases = [
    ...
    { str: '学校技術員研修検討会報告', buf: Buffer.from('GyRCM1g5OzU7PVEwdzgmPSQ4IUYkMnFKczlwGyhC', 'base64') }
  ]

  describe('of textual bodies', () => {
    it.skip('should default to ISO-8859-1', (done) => {
      const buffer = Buffer.from('Content-Type: text/plain\r\n' +
                'Content-Transfer-Encoding: base64\r\n\r\n' +
    ...
    it('should fall back to Iconv', (done) => {
      const buffer = Buffer.from('Content-Type: text/plain; charset=ISO-2022-JP\r\n' +
                'Content-Transfer-Encoding: base64\r\n\r\n' +
    ...
    it.skip('should default to ISO-8859-1 for unrecognized encoding (and emit error)', (done) => {
      const buffer = Buffer.from('Content-Type: text/plain; charset=bad-encoding\r\n' +
                'Content-Transfer-Encoding: base64\r\n\r\n' +
                cases[0].buf.toString('base64'), 'utf8')
  ...
  describe('of headers', () => {
    ...
    it('should use defined charset if provided', (done) => {
      const buffer = Buffer.from('Content-Type: text/plain; charset=UTF-8\r\n' +
                'Subject: ' + cases[0].str + '\r\n' +
                '\r\n' +
                '1234', 'utf8')
  ...
  describe('of header parameters', () => {
    ...
    it('should use mime-word charset if provided', (done) => {
      const buffer = Buffer.from('Content-Type: application/octet-stream\r\n' +
                'Content-Transfer-Encoding: QUOTED-PRINTABLE\r\n' +
