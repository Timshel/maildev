  ...
  simpleParser(input, {}, (err, mail) => {
    if (mail) {
      ...
      mail.headers.received =
        typeof mail.headers.received === "string" ? [mail.headers.received] : mail.headers.received;
      mail.receivedDate = parseReceived(
        mail.date,
        mail.headers.received,
        mail.headers["x-received"],
      );
      ...
      mail.inReplyTo = (mail.inReplyTo ? mail.inReplyTo.match(/<([^<>]*)>/g) : []).map((ref) => {
        return ref.replace(/^<(.*)>$/, "$1");
      });
      ...
      for (const attachment of mail.attachments ?? []) {
        attachment.generatedFileName = generateFileNames(
          fileNames,
          attachment.filename,
